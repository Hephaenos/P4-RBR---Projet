...
int mat_remplie(case_t mat[L][C]){
    ...
    int j;
    for(j=0;j<C;j++){
        if(mat[L-1][j].piece1->type==1){ /* si c'est une pièce bloquante alors la colonne est remplie */
            nb_colonne_remplie++;
        }
        else if(mat[L-1][j].piece1->type==2 && mat[L-1][j].piece2->type==3){ /* S'il y a une piece pleine et creuse alors la colonn est remplie */
            nb_colonne_remplie++;
        }
    }
